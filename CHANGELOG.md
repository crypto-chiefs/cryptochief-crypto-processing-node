...
answer with an order even when the business answer is "no": TRON energy
rental and buying native coins for API credits.

- `client.transactions.estimate()` — signed `POST /v1/transaction/estimate`, the network fee of a transfer **without signing or broadcasting**: the same transfer fields as `sign` minus `urlCallback` (nothing is sent, so no webhooks fire). Answers `estimatedFee` and `required` — the total native coin the from-wallet must hold: fee + value for a native transfer, the fee alone for a token one — each with a USD twin that comes back `''` when no rate is available. `type: 'contract'` is refused with the new `ErrorCode.ContractEstimateUnsupported` (`CONTRACT_ESTIMATE_UNSUPPORTED`): only native and token transfers can be estimated
- On TRON the estimate also carries the fee breakdown: `energyFee` + `bandwidthFee` + `activationFee` = `estimatedFee` (the gross figure, priced at an empty energy pool), plus `energy` units needed, `feeLimit` — the on-chain cap written into the transaction — and `feeExpected`, what the transfer will probably burn given the wallet's current staked/delegated/rented energy. **`feeExpected` is not a funding guarantee**: the pool can expire or be spent before broadcast, so fund `required`, never this figure. Other networks omit all of these
- `client.energy` — TRON energy rental, charged to the same credits balance as the rest of the API. `quote()` (free of charge, `POST /v1/energy/quote`) prices a rental and holds the price until `expiresAt`; `rent()` (`POST /v1/energy/rent`) buys synchronously — by the time it answers, the energy is either delegated to `receiveAddress` or the reason it is not is known; `order(key)` (`POST /v1/energy/order`) reads one rental back by its idempotency key. The quote carries the authoritative integer `priceSun`, the human `priceTrx`, the charge in `credits`, and the saving against burning TRX directly (`burnPriceSun`, `savingTrx`)
- `client.native` — buying native coins (TRX, ETH, BNB, SOL, TON, ...) out of the platform's own liquidity, charged to API credits. `quote()` (free of charge, `POST /v1/native/quote`) prices the coins at the market rate plus the fee of the platform's own transfer to you and holds the price briefly (about 90 seconds, single-use); `buy()` (`POST /v1/native/buy`) buys synchronously; `order(key)` (`POST /v1/native/order`) reads one buy back. The platform sends the coins to any address and pays the transfer itself — `receiveAddress` is any address you want funded
- **A refused or unresolved order is an order, not an exception.** `rent()` and `buy()` recover the order the API returns as the body of a non-2xx business outcome — `refused` on 502 (or 402 when the credits balance is short: nothing was bought or charged, `credits` absent rather than zero) and `unresolved` on 409 (`needsAttention` true: the supplier's answer never arrived, so the energy may already be delegated or the coins already sent — **do not retry**, follow the order with `order()` until it settles). The order carries the reason as `error` and its machine form `errorCode` (wire `error_code`). Only errors with no order to report — a spent quote (`QUOTE_EXPIRED` / `QUOTE_ALREADY_USED`), a gateway failure — throw an `ApiError` as usual
- `opts.idempotencyKey` is **required** by `rent()` and `buy()` and refused locally without one: it is what makes a retry safe. Resubmitting the same key returns the same order instead of buying twice; a new key re-attempts
- An infrastructure failure of the estimate itself — the platform could not reach the chain to price the transaction — answers 502 with error code `ESTIMATE_UNAVAILABLE`, surfaced through `ApiError.code` / `isApiError` like any other gateway code; nothing was signed and nothing was charged
- **CI hygiene:** `.gitattributes` now pins text files to LF checkouts on every platform (`* text=auto eol=lf`, `*.bat` excepted). The signature test vectors are pinned by sha256, and a CRLF checkout on Windows changed their bytes, so the same suite that was green on Linux failed on a Windows clone
